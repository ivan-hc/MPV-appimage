...
$HERE/.local/share/junest/bin/junest proot -n -b "--bind=/home --bind=/home/$(echo $USER) --bind=/media --bind=/mnt --bind=/opt --bind=/usr/lib/locale --bind=/etc --bind=/usr/share/fonts --bind=/usr/share/themes" 2> /dev/null -- mpv "$@"
EOF
chmod a+x ./AppRun
...
_binlibs(){
	...
	SHARESAVED="ffmpeg metainfo ytfzf" # Enter here keywords or file/folder names to save in /usr/lib. By default, the names of the folders that you will save in /usr/share are selected also here.
...
_saveshare(){
	SHARESAVED="ffmpeg metainfo ytfzf"
	mkdir save
...
mv ./*AppImage ./MPV-Media-Player_$VERSION-archimage2.1-4-x86_64.AppImage
